...
export function useInvoices(filters: Filters) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      let query = supabase
        .from('invoices')
        .select('id, invoice_number, status, issue_date, due_date, subtotal, tax_amount, discount_amount, total, notes, client_id, clients(name, company)', { count: 'exact' })
        .eq('user_id', userId)
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        ...
        outstanding: all.filter(i => ['sent', 'overdue'].includes(i.status)).reduce((s, i) => s + i.total, 0),
