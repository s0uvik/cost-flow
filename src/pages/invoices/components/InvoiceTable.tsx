...
const NEXT_STATUSES: Partial<Record<InvoiceStatus, { label: string; value: InvoiceStatus }[]>> = {
...
export function InvoiceTable({ data, count, pageCount, page, isLoading, onDelete, onPageChange }: Props) {
          ...
          <TableBody>
            {isLoading ? (
              ...
              data.map((inv) => {
                ...
                const isDueOverdue = inv.status === 'sent' && isPast(parseISO(inv.due_date))
                return (
                  <TableRow
                    key={inv.id}
                  ...
                  >
                    ...
                    <TableCell className={`text-sm ${isDueOverdue ? 'text-destructive font-medium' : 'text-muted-foreground'}`}>
                    ...
                    <TableCell onClick={(e) => e.stopPropagation()}>
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => navigate({ to: '/invoices/$id', params: { id: inv.id } })}>
                          ...
                          {NEXT_STATUSES[inv.status]?.map((s) => (
                            <DropdownMenuItem
                              key={s.value}
                              onClick={() => statusMutation.mutate({ id: inv.id, status: s.value })}
                            >
                              {s.label}
                            </DropdownMenuItem>
                          ))}
