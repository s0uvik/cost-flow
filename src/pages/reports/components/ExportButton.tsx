  ...
  async function handleExport() {
    ...
    try {
      ...
      for (const tx of rawTransactions) {
        ...
        if (isIncome) { totalIncome += tx.amount; runningBalance += tx.amount }
        else { totalExpense += tx.amount; runningBalance -= tx.amount }
        ...
        if (isIncome) {
          row.getCell(6).font = { color: { argb: 'FF16A34A' }, bold: true, size: 10 }
          row.getCell(6).alignment = { horizontal: 'right' }
        } else {
          row.getCell(7).font = { color: { argb: 'FFDC2626' }, bold: true, size: 10 }
          row.getCell(7).alignment = { horizontal: 'right' }
        }

        // Balance column: green if positive, red if negative
        row.getCell(8).font = { color: { argb: runningBalance >= 0 ? 'FF16A34A' : 'FFDC2626' }, bold: true, size: 10 }
      ...
      const totalsRow = ws.addRow(['', '', '', '', 'Totals', formatINR(totalIncome), formatINR(totalExpense), ''])
      totalsRow.getCell(5).font = { bold: true, size: 10 }
      totalsRow.getCell(6).font = { bold: true, color: { argb: 'FF16A34A' }, size: 10 }
      totalsRow.getCell(6).alignment = { horizontal: 'right' }
      totalsRow.getCell(7).font = { bold: true, color: { argb: 'FFDC2626' }, size: 10 }
      totalsRow.getCell(7).alignment = { horizontal: 'right' }
      totalsRow.eachCell({ includeEmpty: true }, (cell, colNum) => {
        if (colNum >= 5) cell.border = { top: { style: 'thin', color: { argb: 'FFE2E8F0' } } }
      })

      const netRow = ws.addRow(['', '', '', '', 'Net Balance', `${net >= 0 ? '+' : ''}${formatINR(net)}`, '', ''])
      netRow.getCell(5).font = { bold: true, size: 10 }
      netRow.getCell(6).font = { bold: true, color: { argb: net >= 0 ? 'FF16A34A' : 'FFDC2626' }, size: 10 }
