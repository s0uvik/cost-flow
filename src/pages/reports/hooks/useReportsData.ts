...
export function getPresetRange(
  ...
  switch (preset) {
    ...
    case "last_year":
      return {
        from: startOfYear(new Date(now.getFullYear() - 1, 0)),
        to: endOfYear(new Date(now.getFullYear() - 1, 0)),
      };
    case "custom":
      return {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      for (const t of txns.filter((t) => t.type === "expense")) {
        ...
        if (!v) continue;
        if (!vendorMap[v.name])
          vendorMap[v.name] = { name: v.name, amount: 0, count: 0 };
      ...
      for (const inv of invoices.filter((i) => i.status !== "cancelled")) {
        ...
        if (!c) continue;
        if (!clientMap[c.name])
          clientMap[c.name] = { name: c.name, amount: 0, count: 0 };
