    ...
    <Dialog open={!!client} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-sm">
        <DialogHeader>
          ...
          <DialogDescription>
            Delete <span className="font-medium text-foreground">"{client?.name}"</span>?{' '}
            {(client?.invoice_count ?? 0) > 0 && (
              <span className="text-amber-600 font-medium">
                This client has {client?.invoice_count} invoice{client?.invoice_count !== 1 ? 's' : ''} that will be unlinked.{' '}
              </span>
            )}
