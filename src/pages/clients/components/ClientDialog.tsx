...
export function ClientDialog({ open, editing, onClose }: Props) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: { name: '', email: '', phone: '', company: '', address: '', notes: '' },
  })

  useEffect(() => {
    form.reset(editing
      ? { name: editing.name, email: editing.email ?? '', phone: editing.phone ?? '', company: editing.company ?? '', address: editing.address ?? '', notes: editing.notes ?? '' }
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <FormField control={form.control} name="company" render={({ field }) => (
                <FormItem className="col-span-2">
                  ...
                  <FormControl><Input placeholder="Company name" {...field} value={field.value ?? ''} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              ...
              <FormField control={form.control} name="address" render={({ field }) => (
                <FormItem className="col-span-2">
                  ...
                  <FormControl><Input placeholder="Street, City, State" {...field} value={field.value ?? ''} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              ...
              <FormField control={form.control} name="notes" render={({ field }) => (
                <FormItem className="col-span-2">
                  ...
                  <FormControl><Input placeholder="Any additional notes..." {...field} value={field.value ?? ''} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
            </div>
