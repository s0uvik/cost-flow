    ...
    <div className="space-y-6">
      ...
      {isEmpty ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed py-16 text-center">
          ...
          <p className="text-sm text-muted-foreground mb-4">Add your first client to start creating invoices</p>
          <Button onClick={() => setDialogOpen(true)}>
            <Plus className="mr-2 h-4 w-4" />
            Add Client
          </Button>
        ...
        <>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {isLoading
              ...
              : data?.data.map(c => (
                  <ClientCard key={c.id} client={c} onEdit={handleEdit} onDelete={setDeleting} />
                ))
            }
          </div>
      ...
      <ClientDialog open={dialogOpen} editing={editing} onClose={handleCloseDialog} />
