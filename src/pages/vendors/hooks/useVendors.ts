...
export function useVendors(filters: Filters) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      let query = supabase
        .from('vendors')
        .select('id, name, email, phone, address, category_id, notes, created_at, categories(name, color)', { count: 'exact' })
        .eq('user_id', userId)
        .order('name')
      ...
      for (const t of txns ?? []) {
        ...
        if (!statsMap[t.vendor_id]) statsMap[t.vendor_id] = { count: 0, total: 0 }
