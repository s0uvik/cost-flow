...
const AVATAR_COLORS = [
  'bg-violet-500', 'bg-cyan-500', 'bg-rose-500', 'bg-lime-500',
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-3 min-w-0">
          <div className={`size-10 shrink-0 rounded-full ${getAvatarColor(vendor.name)} flex items-center justify-center text-white text-sm font-bold`}>
      ...
      <div className="space-y-1.5">
        {vendor.email && (
          <p className="text-xs text-muted-foreground flex items-center gap-1.5 truncate">
            <Mail className="h-3.5 w-3.5 shrink-0" />{vendor.email}
          </p>
        )}
        {vendor.phone && (
