...
export function useDashboardStats() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return { income, expenses, net: income - expenses, outstanding, outstandingCount: invoices.length }
    },
  })
}
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const [budgetsRes, txRes] = await Promise.all([
        supabase
          .from('budgets')
          .select('id, name, amount_limit, period, category_id, categories(name, color)')
          .eq('user_id', userId!)
      ...
      return budgets.map(b => {
        ...
        return { ...b, spent, percentage: Math.min((spent / b.amount_limit) * 100, 100) }
      })
    },
  })
}
