          ...
          : data?.map((tx) => {
              ...
              return (
                <div key={tx.id} className="flex items-center gap-3">
                  <div className={`flex size-8 shrink-0 items-center justify-center rounded-full ${isIncome ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                    {isIncome
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      ...
                      {cat && (
                        <Badge variant="secondary" className="text-xs px-1.5 py-0 h-4" style={{ backgroundColor: cat.color + '22', color: cat.color }}>
                          {cat.name}
                        </Badge>
                      )}
                    </div>
