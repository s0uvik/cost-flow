...
export function BudgetCard({ budget, onEdit, onDelete }: Props) {
  ...
  return (
    <div
      className={`rounded-xl border p-5 space-y-4 transition-colors hover:bg-muted/30 ${isOver ? "border-destructive/40 bg-destructive/5" : ""}`}
    >
      {/* Header */}
      ...
      <div className="space-y-1.5">
        <div className="flex justify-between text-sm">
          ...
          <span
            className={`font-semibold ${isOver ? "text-destructive" : isWarning ? "text-amber-600" : ""}`}
          >
